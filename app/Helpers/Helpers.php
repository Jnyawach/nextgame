...
if (!function_exists('livescore')){
    function livescore($date = null)
    {
        $date = empty($date) ? Carbon::now() : Carbon::createFromDate($date);
        $startOfCalendar = $date->copy()->firstOfMonth()->startOfWeek(Carbon::SUNDAY);
        $endOfCalendar = $date->copy()->lastOfMonth()->endOfWeek(Carbon::SATURDAY);
        ...
        while($startOfCalendar <= $endOfCalendar)
        {
            ...
            $html .= '<span class="day text-center '.$extraClass.'"><a href="/livescores/football/'.$startOfCalendar->format('Y-m-d').'" title="Football Prediction"><span class="content">' . $startOfCalendar->format('j') . '</span></a> </span>';
            $startOfCalendar->addDay();
        }
        $html .= '</div></div>';
        return $html;
